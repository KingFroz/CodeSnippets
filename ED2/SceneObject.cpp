...
void SceneObject::ConstrainToAABB(const AABB& aabb)
{
	...
	for (int i = 0; i < 3; i++)
	{
		if (GetBounds().m_Center.v[i] + GetBounds().m_Radius > aabb.max.v[i])
		{
			GetBounds().m_Center.v[i] = aabb.max.v[i] - GetBounds().m_Radius;
		}

		else if (GetBounds().m_Center.v[i] - GetBounds().m_Radius < aabb.min.v[i] )
		{
			GetBounds().m_Center.v[i] = aabb.min.v[i] + GetBounds().m_Radius;
		}
	}
}
