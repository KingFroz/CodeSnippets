...
MY_MATRIX_4X4 BuidRotationMatrixOnAxisZ(float angle)
{
	...
	identity.p11 = (float)cos(angle * PI / 180);
	identity.p12 = (float)-sin(angle * PI / 180);
	identity.p13 = 0;

	identity.p21 = (float)sin(angle * PI / 180);
	identity.p22 = (float)cos(angle * PI / 180);
...
MY_MATRIX_4X4 YRotation(float angle)
{
	MY_MATRIX_4X4 identity = MatrixIdentity();
	identity.p11 = (float)cos(angle * (PI / 180));
	identity.p13 = (float)sin(angle * (PI / 180));
	identity.p22 = 1;

	identity.p31 = (float)-sin(angle * (PI / 180));
	identity.p33 = (float)cos(angle * (PI / 180));
...
MY_MATRIX_4X4 XRotation(float angle)
{
	...
	identity.p22 = (float)cos(angle * (PI / 180));
	identity.p23 = (float)-sin(angle * (PI / 180));

	identity.p32 = (float)sin(angle * (PI / 180));
	identity.p33 = (float)cos(angle * (PI / 180));
...
float ILE(MY_VERTEX v0, MY_VERTEX v1, MY_VERTEX v2) {
	float x, y, sq, ile;

	x = v2.axis[0] - v1.axis[0];
	y = v1.axis[1] - v2.axis[1];

	sq = v1.axis[0] * v2.axis[1] - v1.axis[1] * v2.axis[0];
	ile = (y * v0.axis[0] + x * v0.axis[1] + sq);

	return ile;
}

MY_VERTEX Barycentric(const MY_VERTEX _v, const MY_VERTEX tri[3])
{
	float _beta, _gam, _alp;

	_alp = ILE(tri[0], tri[2], tri[1]);
	_beta = ILE(tri[1], tri[0], tri[2]);
	_gam = ILE(tri[2], tri[1], tri[0]);

	float  b, y, a;

	a = ILE(_v, tri[2], tri[1]);
	b = ILE(_v, tri[0], tri[2]);
	y = ILE(_v, tri[1], tri[0]);
